...
public class PositionRepository(
    ...
    IMapper mapper) : IPositionRepository
{
    ...
    {
        var entity = await context.Positions.FirstOrDefaultAsync(p => p.Id == id, ct)
            ?? throw new NotFoundException("Position note not found");

        if (model.CellId.HasValue)
        {
            entity.CellId = model.CellId.Value;
        }
        ...
        {
            entity.SellingPrice = model.SellingPrice.Value;
        }
