...
public class PaymentNote
{
    ...
    public static (PaymentNote? paymentNote, List<string> errors) Create(
        long id, long billId, DateTime date, decimal amount, PaymentMethodEnum methodId)
    {
        ...
        var billIdError = DomainValidator
            .ValidateId(billId, "billId");
        if (!string.IsNullOrEmpty(billIdError))
        {
            errors.Add(billIdError);
        }

        var methodIdError = DomainValidator
            .ValidateId(methodId, "methodId");
        if (!string.IsNullOrEmpty(methodIdError))
        {
            errors.Add(methodIdError);
        }
