...
public class Bill
{
        ...
        DateOnly? actualBillDate)
    {
        ...
        var createdAtError = DomainValidator
            .ValidateDate(createdAt, "createdAt");
        if (!string.IsNullOrEmpty(createdAtError))
        {
            errors.Add(createdAtError);
        }

        var amountError = DomainValidator
        ...
        {
            var actualDateError = DomainValidator
                .ValidateDate(actualBillDate, "Actual bill date"); 
            if (!string.IsNullOrEmpty(actualDateError))
            {
                errors.Add(actualDateError);
            }

            var actualDateRangeError = DomainValidator
                .ValidateDateRange(createdAt, actualBillDate);
            if (!string.IsNullOrEmpty(actualDateRangeError))
            {
                errors.Add(actualDateRangeError);
            }
        }
