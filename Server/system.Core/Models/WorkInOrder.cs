...
public class WorkInOrder
{
        ...
        long id, long orderId, long jobId, int workerId, WorkStatusEnum statusId, decimal timeSpent)
    {
        ...
        var jobIdError = DomainValidator
            .ValidateId(jobId, "jobId");
        if (!string.IsNullOrEmpty(jobIdError))
        {
            errors.Add(jobIdError);
        }
        ...
        var statusErrorId = DomainValidator
            .ValidateId(statusId, "status");
        if (!string.IsNullOrEmpty(statusErrorId))
        {
            errors.Add(statusErrorId);
        }

        var timeSpentError = DomainValidator
            .ValidateMoney(timeSpent, "time");
        if (!string.IsNullOrEmpty(timeSpentError))
        {
            errors.Add(timeSpentError);
        }
