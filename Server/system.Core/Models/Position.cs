...
public class Position
{
        ...
        long id, long partId, int cellId, decimal purchasePrice, decimal sellingPrice, decimal quantity)
    {
        ...
        var partIdError = DomainValidator
            .ValidateId(partId, "partId");
        if (!string.IsNullOrEmpty(partIdError))
        {
            errors.Add(partIdError);
        }

        var cellIdError = DomainValidator
            .ValidateId(cellId, "cellId");
        if (!string.IsNullOrEmpty(cellIdError))
        {
            errors.Add(cellIdError);
        }

        var purchaseError = DomainValidator
            .ValidateMoney(purchasePrice, "purchasePrice");
        if (!string.IsNullOrEmpty(purchaseError))
        {
            errors.Add(purchaseError);
        }

        var sellingError = DomainValidator
            .ValidateMoney(sellingPrice, "sellingPrice");
        if (!string.IsNullOrEmpty(sellingError))
        {
            errors.Add(sellingError);
        }
