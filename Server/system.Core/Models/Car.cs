...
public class Car
{
        ...
        int mileage)
    {
        ...
        var ownerIdError = DomainValidator
            .ValidateId(ownerId, "ownerId");
        if (!string.IsNullOrEmpty(ownerIdError))
        {
            errors.Add(ownerIdError);
        }
        ...
        var brandError = DomainValidator
            .ValidateString(brand, ValidationConstants.MAX_BRAND_LENGTH, "brand");
        if (!string.IsNullOrEmpty(brandError))
        {
            errors.Add(brandError);
        }

        var modelError = DomainValidator
            .ValidateString(model, ValidationConstants.MAX_MODEL_LENGTH, "model");
        ...
        var VINError = DomainValidator
            .ValidateString(vinNumber, ValidationConstants.MAX_VIN_LENGTH, "VIN");
        ...
        var stateNumberError = DomainValidator
            .ValidateString(stateNumber, ValidationConstants.MAX_STATE_NUMBER_LENGTH, "stateNumber");
        if (!string.IsNullOrEmpty(stateNumberError))
        {
            errors.Add(stateNumberError);
        }
