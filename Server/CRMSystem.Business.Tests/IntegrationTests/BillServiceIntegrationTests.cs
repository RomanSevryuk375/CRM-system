...
public class BillServiceIntegrationTests : BaseIntegrationTest, IClassFixture<IntegrationTestFactory>
{
    ...
    [Fact]
    public async Task RecalculateAmount_ShouldUpdateBillDebt_WhenPartsAndWorksExist()
    {
        var userWorker = new UserEntity
        {
            RoleId = (int)RoleEnum.Worker,
        ...
        var paymentNote = new PaymentNoteEntity
        {
            BillId = bill.Id,
        ...
        var dept = await _billService.FetchDebtOfBill(bill.Id, CancellationToken.None);
